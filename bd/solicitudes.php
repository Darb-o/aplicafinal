<?php
    ...
    switch($opcion){
        ...
        case 1://insertar valores nombre de grupo y descripcion del grupo en la tabla grupo
            ...
            $sql="select * from grupo by id_grupo desc limit 1";
            $res=$link->prepare($sql);
            $res->execute();
        ...
        case 5://insertar valores nombre de producto, precio, descripcion del producto, imagen del producto y grupo del producto en la tabla productos
            ...
            $res->execute();
            //mostrar los datos insertados
            $sql="select * from productos by id_producto desc limit 1";
            $res=$link->prepare($sql);
            $res->execute();
            $data=$res->fetchAll(PDO::FETCH_ASSOC);
            break;
        ...
        case 11://insertar valores id de producto, descuento, fecha de inicio, fecha final en la tabla descuentos
            ...
            $res->execute();
            //mostrar los datos insertados
            $sql="select * from descuentos by id_descuento desc limit 1";
            $res=$link->prepare($sql);
            $res->execute();
            $data=$res->fetchAll(PDO::FETCH_ASSOC);
            break;
        ...
        case 15://consulta productos
            $sql="select id_producto,nombre_produc from productos where estado_producto='1'";
            $res=$link->prepare($sql);
            $res->execute();
            $data=$res->fetchAll(PDO::FETCH_ASSOC);
            break;
        ...
        case 18://
            ...
            $data=$res->fetchAll(PDO::FETCH_ASSOC);
            break;
        case 19://consultar si existe una factura para el dia actual para el usuario talvez borrar
            $sql="select precio from productos where id_producto='$id_p'";
            $res=$link->prepare($sql);
            $res->execute();
            $data=$res->fetchAll(PDO::FETCH_ASSOC);
            break;
        case 20://crear una nueva factura
            ...
            $res=$link->prepare($sql);
            $res->execute();
            $sql="select id_factura from pedido where correo='$usuario' and estado_pedido is NULL";
            $res=$link->prepare($sql);
            $res->execute();
            $data=$res->fetchAll(PDO::FETCH_ASSOC);
            break;
        case 21://ingresar datos en orden_producto
            $sql="insert into orden values ('$id_factura','$id_p','$unidades','$subtotal')";
            //$sql="insert into orden (id_producto,id_orden,unidades) values ('$id_p','$id_orden','$unidades')";
            $res=$link->prepare($sql);
            $res->execute();
            $data=$res->fetchAll(PDO::FETCH_ASSOC);
            break;
        ...
        case 24://
            ...
            break;
        case 25://
            $sql="select productos.id_producto, productos.nombre_produc,productos.precio, orden.unidades ,orden.subtotal FROM productos INNER JOIN orden ON orden.id_producto=productos.id_producto WHERE orden.id_factura='$id_factura'";
            $res=$link->prepare($sql);
            $res->execute();
            $data=$res->fetchAll(PDO::FETCH_ASSOC);
            break;
        case 26://
            ...
            break;
        case 27://
            $sql="select id_factura, correo,fecha_pedido,tipo_pedido,valor_total FROM pedido WHERE estado_pedido='$estadoP'";
            $res=$link->prepare($sql);
            $res->execute();
            $data=$res->fetchAll(PDO::FETCH_ASSOC);
            break;
        case 28://
            ...
            break;
        case 29://
            $sql="select * from descuentos where '$fechaCompra' between fecha_i and fecha_f";
            $res=$link->prepare($sql);
            $res->execute();
            $data=$res->fetchAll(PDO::FETCH_ASSOC);
            break;
        case 30://
            $sql="select pedido.id_factura, pedido.correo,pedido.fecha_pedido,pedido.tipo_pedido,pedido.valor_total, orden.id_orden FROM pedido INNER JOIN orden ON orden.id_factura=pedido.id_factura WHERE pedido.estado_pedido='$estadoP' and pedido.fecha_pedido between '$fecha_i' and '$fecha_f'";
            $res=$link->prepare($sql);
            $res->execute();
            $data=$res->fetchAll(PDO::FETCH_ASSOC);
            break;
        case 31://
            $sql="select correo,nombre,direccion from usuario where correo='$correo'";
            $res=$link->prepare($sql);
            $res->execute();
            $data=$res->fetchAll(PDO::FETCH_ASSOC);
            break;
        case 32://
            $sql="select id_factura, correo, fecha_pedido, tipo_pedido, valor_total FROM pedido WHERE estado_pedido='1' or estado_pedido='3'";
            $res=$link->prepare($sql);
            $res->execute();
            $data=$res->fetchAll(PDO::FETCH_ASSOC);
            break;
        case 33://
            $sql="select id_producto FROM orden";
            $res=$link->prepare($sql);
            $res->execute();
            $data=$res->fetchAll(PDO::FETCH_ASSOC);
            break;
        case 34:
            $sql="update pedido set estado_pedido='2' where id_factura='$id_factura'";
            $res=$link->prepare($sql);
            $res->execute();
            $data=$res->fetchAll(PDO::FETCH_ASSOC);
        break;
